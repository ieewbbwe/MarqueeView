...
public class CustomerTextView extends AppCompatTextView {
	...
	private ValueAnimator mSwitchAnimator = new ValueAnimator();
	...
	private void init() {
		...
		mValueAnimator.setDuration(mSwitchDuration);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mContentList.size() > 1) {
			...
			// 计算绘制区域
			contentPaint.getTextBounds(mCurrentString, 0, mCurrentString.length(), mComputeRect);
			currentStringWidth = mComputeRect.width();
			currentStringHeight = mComputeRect.height();
			mCurrentY = currentStringHeight + paddingTop;

			int width = getWidth() - getPaddingLeft() - getPaddingRight();
			Log.d("keep2iron","isStart : " + mValueAnimator.isStarted());
			//超过一屏
			if (currentStringWidth > width && !isAnimStart) {
				mValueAnimator.setIntValues(0, currentStringWidth - width);
